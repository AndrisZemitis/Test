...
if($_POST['labojuma_sadala'] == "update_record"){
  ...
  switch($head_name){
    case 'clsKraujmers':
      ...
      $date_pavadzime = $_POST['date_pavadzime'];
...
class loadDataForSymbol {
  ...
  function loadKraujmersHead(){
    ...
    $mysql_query_txt = "SELECT `id`, `date_upload`, `date_pavadzime`, `darba_uzdevuma_num`, `uzm_vieta`, `buyer`, `seller`, `reciever`, `transport`, `driver` FROM `{$tmp_table}` WHERE `opcija` = 'A' {$where_state} ORDER BY `id` DESC";
    ...
    $this->data_structure['data_head'] = array(0 => 'Nr.', 1 => 'Fails ielasīts', 2 => 'Pavadzīmes datums', 3 => 'Darba uzd. nr.', 4 => 'Uzm. vieta', 
                                                5 => 'Pircējs', 6 => 'Pārdevējs' , 7 => 'Saņēmējs', 8 => 'Transports', 9 => 'Šoferis', 
  ...
  function getTranslation(){
    ...
    $this->translation['date_pavadzime'] = "Pavadzīmes datums";
  ...
  function updatedStatus($id){
    $updated_status_text = "SELECT `eksporta_datums`, `date_pavadzime` FROM `t_h_kraujmers` WHERE `id` = {$id}";
    ...
    while($updated_status_arr = mysql_fetch_assoc($updated_status_query)){
      ...
      $date_pavadzime = $updated_status_arr['date_pavadzime'];
    }
    ...
    if($date_pavadzime == '0000-00-00 00:00:00'){
      $result['pavadzime'] = 1;
    }
  ...
  //	if ($m['skatits'] == 0){$color='#FFCCCC';}
   ...
   ?>
    <tr bgcolor='<?=$color?>'>
      ...
      <?
      $date_ver_arr = $loadData->updatedStatus($tmp_body['id']);
      $updated = $date_ver_arr['upload'];
      $pavadzimes_datums = $date_ver_arr['pavadzime'];

      if($head_name == 'clsKraujmers' && $updated == 0 && $pavadzimes_datums == 0){?>
        <td id="<?=$tmp_body['id']?>" align=center>&nbsp;&nbsp;<a href="#" onclick="exportResult(<?=$tmp_body['id']?>);">Eksportēt</a></td>
      <? }elseif($head_name == 'clsKraujmers' && $updated == 0 && $pavadzimes_datums == 1){ ?>
        <td align=center>Nekorekti dati</td>
