...
/*funkcija nomaina datubāzes tabulas ar padoto ieraksta ID "skatits" lauku uz 1, atgriez 1 ja viss OK
function ieraksts_skatits($skatits_id)
	{
	echo"UPDATE `papirmalka` SET skatits="1" WHERE id=".$skatits_id."";
	$result = mysql_query("UPDATE `papirmalka` SET skatits="1" WHERE id=".$skatits_id."");
	return 1;
	}
*/
// atgriez datumu un laiku ar -1 stundu ievadītam datumam fromā Y-m-d H:i:s piem. 2005-06-23 12:04:02 jeb MySQL datubāzes defult formā
function stundas_atnemsana($new_time)
{
	$new_time = explode(' ',$new_time);
		$new_date_final = explode('-',$new_time[0]);
		$new_time_final = explode(':',$new_time[1]);
	$new_time_final[0]=$new_time_final[0]-1;
	$F_time=date("Y-m-d H:i:s", mktime($new_time_final[0],$new_time_final[1],$new_time_final[2],$new_date_final[1],$new_date_final[2],$new_date_final[0]));
	return $F_time;
}

// atgriež nakamās dienas sākumu
function nakama_diena($dp)
{
	if (date("d",$dp) == date("d",mktime(0,0,0,date("m",$dp),date("d",$dp)+1,date("Y"),$dp)))
	{
		// gadījumā ja šajā dienā notiek pāreja uz vasaras laiks, notiek gļuks
		// šeit ja pieskaitot dienu tomēr datums nemainās, ielikts cikls
		// kurš pieskaita minūti datumam tik ilgi kamēr datums nomainās.
		$datums = date("d",$dp);
		while (date("d",$dp) == $datums)
			$dp = $dp + 60;
		return $dp;
	}
	else
		return mktime(0,0,0,date("m",$dp),date("d",$dp)+1,date("Y"),$dp);
}

//funkcija sms suutishanai uz LMT, TELE2, TRIATEL
function sms_send($ident,$teksts)
{
	mail($ident, 'VMF', $teksts);
...
function menesa_sakums()
{
	return mktime(0,0,0,date('m'),1,date('Y'));
}

function menesa_beigas()
{
...
{
	if ($tabula != '' || $id == '') {
		$rt = mysql_query("select * from $tabula where 1=2");
	} elseif ($tabula == '') {
		$rt = mysql_query("select * from lietotaji where 1=2");
	} else {
...
function atskaites_stundas2($atskaite_id,$sakums,$beigas)
{
	...
	//echo "<_".sqltime($t10)." ".sqltime($t20)."_><br>";

	// cikls pa dienām
	for ($i = $t10;$i<=$t20;$i=$i+(60*60*24))
	{	
			// nosakam cik strādāts konkrētajā dienā
			//echo "<_(".sqltime($t10)." ".sqltime($t20).") ".sqltime($i)."_><br><br>";
			...
			if (sqltime($i) == "2006-03-26 00:00:00")
		    {
				//echo sqltime($i);
				$t1apr_d = $t1apr_d - (60*60);
				$t2apr_d = $t2apr_d - (60*60);
				$t2apr = $t2apr - (60*60);
				//echo sqltime($t1apr_d)." ".sqltime($t2apr_d)." ".sqltime($t2apr)."-><br>";
			}; 
			...
			if (sqldate($i) == '2006-03-26')
			{
				if (sqltime($t1) <= '2006-03-26 03:00:00' and sqltime($t2) >= '2006-03-26 03:00:00')
				{
					$stundas_nakts = $stundas_nakts - 1;
					echo "test";
				}
