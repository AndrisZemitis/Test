...
{ $h=(int)$h;
  ...
  $t[64]="Atskaite(Timberex Group)";   
	...
	<table>
	<tr><td><a href='http://www.vmf.lv/main_menu.php'><img src=images/logo.gif border=0></a>&nbsp;&nbsp;</td>
	<td><font color=white size=4>
	[ <a href=batch_upload.php?h=1 class='head'><?=$t[1]?></a> ]&nbsp;
	[ <a href=batch_select.php?h=2 class='head'><?=$t[2]?></a> ]&nbsp;
...
//  die('Hello');
  $pavadzime=trim($pavadzime);
	$value=trim($value);
	$r = mysql_query("select $value from `$tbl` where pavadzime = '$pavadzime'");
	if ($m = mysql_fetch_array($r))
		return $m[$value];
	else
		return false;
...
{ $pavadzime=trim($pavadzime);
	$r = mysql_query("select min(datums_laiks) as dat from vikawood_balkis,vikawood_pavadzime where vikawood_balkis.pavadzime =vikawood_pavadzime.id and vikawood_pavadzime.pavadzime = '$pavadzime'");
	if ($m = mysql_fetch_array($r))
		return $m['dat'];
	else
		return false;
...
//-----------------------------------------------------------------------------------
function get_AtskVeidsByPlaceCode($placeNum){
  switch($placeNum){
    case 2010:
      $veids = "2010";
      break;
...
function tmp_getTableName($atskaites_veids){
  switch($atskaites_veids){
      ...
      break;
      case "kubikmetrs":
       $tabulaName = "kubikmetrs_";
      break;
      case "jekabpils_mr":
       $tabulaName = "jekabpils_mr_";
      break;
      case "latvani":
       $tabulaName = "latvani_";
      break;
      case "timberex_group":
       $tabulaName = "timberex_group_";
      break;
  }
  return $tabulaName;
}
