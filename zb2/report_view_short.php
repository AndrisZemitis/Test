...
switch($atskaite_veids){
    ...
    break;
    case "nelss":
      include ("c_report.inc_short.php");
      $pavadzimes_tabula = "nelss_pavadzime";
	  $atskaites_kods = 18;
    break;
    case "vika":
      include ("c_report.inc_vika.php");
      $pavadzimes_tabula = "vikawood_pavadzime";
	  $atskaites_kods = 2;
    break;
    case "laiko":
    ...
    break;
    case "osukalni":
      include ("c_report.inc_osukalni.php");
      $pavadzimes_tabula = "osukalni_pavadzime";
      $atskaites_kods = 29;
    break;
	default:
		$output_xml_dir = "other";
}
...
//        $output = $output . "	<head>";
			...
			if ($output_pdf) {
				...
				$output = $output . "	<table cellspacing=0 cellpadding=0>";
				 ...
				 if ($_POST['pircejs_regnum']) { 
				 $output = $output . "<tr><td align=right>Pircējs Reg.Num:&nbsp;</td><td>".$_POST['pircejs_regnum']."</td>";
				 } 
				 if ($_POST['pardevejs_head']) { 
				 $output = $output . "<tr><td align=right>".$translate['pardevejs'][$lang].":&nbsp;</td><td>".$_POST['pardevejs_head']."</td>";
				 } 
				 if ($_POST['pardevejs_regnum']) { 
				 $output = $output . "<tr><td align=right>Pārdevējs Reg.Num:&nbsp;</td><td>".$_POST['pardevejs_regnum']."</td>";
				 } 
...
//        set_param('terminsh',$_POST['terminsh_head']);
	...
	if (!$output_pdf) {
		if($output_xml){
			$xml=$C_REP->GetXML($pavadzimes_tabula);
		} else {
	...
	if ($output_xml) {
		...
		else
		{
			...
			$output .= "Izveidots fails ../batch/xml/".$output_xml_dir."/".$pavXMLNum.".xml<BR>";
		}
		echo $output;
	} elseif ($output_pdf) {
	...
	/*Atskaite tiek pievienota papildus informācija par brāki, izņemot VIKAWOOD atskaiti*/  
	  if($atskaite_veids == 'vika' || $atskaite_veids == 'gaujaskoks' || $atskaite_veids == 'smiltene' || $atskaite_veids == 'bsw' || $atskaite_veids == 'vudlande' || $atskaite_veids == 'pata_ab' || $atskaite_veids == 'nelss' || $atskaite_veids == 'piebalgas' || $atskaite_veids == 'latvijas_finieris' || $atskaite_veids == 'stora_enso'){
