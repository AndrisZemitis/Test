...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
			...
			if(trim($this->MyPOST['pavadzime'])!='')
				...
				if	(!GetFromPavadzime($this->MyPOST['pavadzime'],'pavadzime','bsw_pavadzime') ) $this->ERRORS .= "Norādīts NEEKSISTĒJOŠS PAVADZĪMES numurs!<br><br>"; 
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_2"],2);
				...
				if($this->MyPOST["gtype".$this->grup.($i+1)]=="bsw_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['bsw_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
function print_level($level,$query,&$summa_p,$irbrakis_p,$suga,$id)
{
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'gar_pec_red' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'bsw_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'skira' || $lauks=='suga' || $lauks=='iecirknis')
...
//Smiltene Impex ipasa grupesana pec mind_miza ... 
 ...
 //       $this->MyPOST["gtype".$this->grup.$level] = 'mind_miza';
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . " >= " . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . " <= " . $mbounds[1] . " AND bsw_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . " >= " . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . " <= " . $high_range_brakis . " AND bsw_balkis_temp.import_type = 'zbm'))";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Saakums////////////////////////////////////////////////////////////////////////////////////
/*Query izvilksana*/  
      $query__="select mind_pirms_red as tievgalis,bsw_balkis_temp.suga as suga, mind_miza as DBruto,bsw_balkis_temp.id as bb_id, garums as LBruto, mind_miza as DPecReduc, gar_pec_red as LPecReduc, brakis, bsw_pavadzime.cenu_matrica as cm" . $myquery;
