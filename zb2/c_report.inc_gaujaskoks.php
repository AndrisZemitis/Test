...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
			...
			if(trim($this->MyPOST['pavadzime'])!='')
				{
				if	(!GetFromPavadzime($this->MyPOST['pavadzime'],'pavadzime','gaujaskoks_pavadzime') ) $this->ERRORS .= "Norādīts NEEKSISTĒJOŠS PAVADZĪMES numurs!<br><br>"; 
	...
	{	
		...
		{
			$pav_test_query = "select count(*) as x from gaujaskoks_pavadzime where pavadzime = '".trim($this->MyPOST['pavadzime'])."'";
		...
		if ($this->MyPOST[datums_no_diena]!='' || $this->MyPOST[pavadzime_id]) { // 
			$query = " from gaujaskoks_balkis_temp INNER JOIN gaujaskoks_pavadzime ON gaujaskoks_balkis_temp.pavadzime = gaujaskoks_pavadzime.id WHERE 1=1 ";
		} else {
			$query = " from gaujaskoks_balkis INNER JOIN gaujaskoks_pavadzime ON gaujaskoks_balkis.pavadzime = gaujaskoks_pavadzime.id WHERE 1=1 ";
		}
		...
		if ($this->MyPOST[piegad_grupa])
			$query=$query . " and gaujaskoks_pavadzime.piegad_grupa like '".$this->MyPOST[piegad_grupa]."%'";
		if ($this->MyPOST[piegadataju_kods])
			$query=$query . " and gaujaskoks_pavadzime.piegadataju_kods = ".$this->MyPOST[piegadataju_kods];
		if ($this->MyPOST[pavadzime])
			$query=$query . " and gaujaskoks_pavadzime.pavadzime = '".$this->MyPOST[pavadzime]."'";
		if ($this->MyPOST[pavadzime_id])
			$query=$query . " and gaujaskoks_pavadzime.id = '".$this->MyPOST[pavadzime_id]."'";
		if ($this->MyPOST[suga]!='') 
		...
		$query2 = str_replace("gaujaskoks_balkis_temp","gaujaskoks_balkis",$query);

		if ($this->MyPOST[datums_no_diena]!='') {
			$this->DB->query("DELETE FROM gaujaskoks_balkis_temp");
			$this->DB->query("INSERT INTO gaujaskoks_balkis_temp (SELECT gaujaskoks_balkis.* " . $query2 . ")");
			$query = " from gaujaskoks_balkis_temp INNER JOIN gaujaskoks_pavadzime ON gaujaskoks_balkis_temp.pavadzime = gaujaskoks_pavadzime.id WHERE 1=1 ";
		}
		if ($this->MyPOST[pavadzime]) {
			$this->DB->query("DELETE FROM gaujaskoks_balkis_temp");
			...
			for ($i=0;$i<$this->lsk;$i++)
			{
				...
				if($this->XML_Variants)
				{
					...
					if($this->MyPOST["gtype".$this->grup.($i+1)]=='mind_pirms_red' ) {
						...
						$this->MyPOST["gvalues".$this->grup.($i+1)]=str_replace("b0-99","b60-99",$this->MyPOST["gvalues".$this->grup.($i+1)]);
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_2"],2);
				...
				if($this->MyPOST["gtype".$this->grup.($i+1)]=="gaujaskoks_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['gaujaskoks_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
function print_level($level,$query,&$summa_p,$irbrakis_p,$suga,$id)
{
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'gaujaskoks_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'skira' || $lauks=='suga' || $lauks=='iecirknis')
...
//pievienots 17.07.2007
			if ($this->MyPOST["gtype".$this->grup.$level] == 'mind_pirms_red') {
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $mbounds[1] . " AND gaujaskoks_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $high_range_brakis . " AND gaujaskoks_balkis_temp.import_type = 'zbm'))";
			} else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Saakums////////////////////////////////////////////////////////////////////////////////////
/*Query izvilksana*/  
      $query__="select mind_pirms_red as DBruto,gaujaskoks_balkis_temp.id as balkis_id, garums as LBruto, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, gaujaskoks_pavadzime.cenu_matrica as cm " . $myquery;
...
//        if (strtoupper($this->MyPOST['pieg_lig_num'])=='VIKA WOOD') {
		if ($this->MyPOST['is_vika'] == 'on') {
          ...
          $diametrs_zem_desmit = $DBrutoNoap * 1000;

/*          
          if ($m['LBruto'] == 374 && $m['DBruto'] == 113) {
          echo "ID = ".$balkis_id.'<br />';          
          
          echo "<---------------Garuma vienības------------------>",'<br />';
          echo "DBrutoNoap = ".$DBrutoNoap.'<br />';
          echo "LBruto = ".$LBruto.'<br />','<br />';
          echo "DBrutoNoap = ".$DBrutoNoap.'<br />';
          echo "LNomBruto = ".$LNomBruto.'<br />','<br />';
          


          echo "<---------------Tilpuma vienības------------------>",'<br />';
				...
				if (!$irbrakis) {
					...
					else
					{
						...
						if ($this->MyPOST['braka_virsmers'] == 'on') {
                              ...
                              if ($m['LBruto'] >= 246 && $m['LBruto'] < 306){
                                          
                                    $LNomBrutoBrakim = 240/100;
                                    $tilpums_bruto_brakim = f2($DBrutoNoap,$LNomBrutoBrakim,$rauk_koef,$koeficients,$gostu_tabula);
                                    $VBrakis = $tilpums_bruto_brakim;
                                    $SVVirsm += $tilpums_bruto - $tilpums_bruto_brakim;
                              }else if($m['LBruto'] < 246){
                                   $tilpums_neto = 0;
                                   $tilpums_diametrs_redukcija = 0;
                                   $tilpums_garuma_redukcija = 0;
                                   $VBrakis = 0;
                                   $tilpums_virsmers = 0;
                              }else if($m['LBruto'] >= 306 && $m['LBruto'] < 366){
                                    $LNomBrutoBrakim = 300/100;
                                    $tilpums_bruto_brakim = f2($DBrutoNoap,$LNomBrutoBrakim,$rauk_koef,$koeficients,$gostu_tabula);
                                    $VBrakis = $tilpums_bruto_brakim;
                                    $SVVirsm += $tilpums_bruto - $tilpums_bruto_brakim;
                              }else{
