...
$VIKA_braki = gAtskaite::getVIKABrakiKurekss();
...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
			...
			if(trim($this->MyPOST['pavadzime'])!='')
				{
				if	(!GetFromPavadzime($this->MyPOST['pavadzime'],'pavadzime','k_pavadzime') ) $this->ERRORS .= "Norādīts NEEKSISTĒJOŠS PAVADZĪMES numurs!<br><br>"; 
	...
	{	
		...
		{
			$pav_test_query = "select count(*) as x from k_pavadzime where pavadzime = '".trim($this->MyPOST['pavadzime'])."'";
		...
		if ($this->MyPOST[datums_no_diena]!='' || $this->MyPOST[pavadzime_id]) { // 
			$query = " from k_balkis_temp INNER JOIN k_pavadzime ON k_balkis_temp.pavadzime = k_pavadzime.id WHERE 1=1 ";
		} else {
			$query = " from k_balkis INNER JOIN k_pavadzime ON k_balkis.pavadzime = k_pavadzime.id WHERE 1=1 ";
		}
		...
		if ($this->MyPOST[piegad_grupa])
			$query=$query . " and k_pavadzime.piegad_grupa like '".$this->MyPOST[piegad_grupa]."%'";
		if ($this->MyPOST[piegadataju_kods])
			$query=$query . " and k_pavadzime.piegadataju_kods = ".$this->MyPOST[piegadataju_kods];
		if ($this->MyPOST[pavadzime])
			$query=$query . " and k_pavadzime.pavadzime = '".$this->MyPOST[pavadzime]."'";
		if ($this->MyPOST[pavadzime_id])
			$query=$query . " and k_pavadzime.id = '".$this->MyPOST[pavadzime_id]."'";
		if ($this->MyPOST[suga]!='') 
		...
		$query2 = str_replace("k_balkis_temp","k_balkis",$query);

		if ($this->MyPOST[datums_no_diena]!='') {
			$this->DB->query("DELETE FROM k_balkis_temp");
			$this->DB->query("INSERT INTO k_balkis_temp (SELECT k_balkis.* " . $query2 . ")");
			$query = " from k_balkis_temp INNER JOIN k_pavadzime ON k_balkis_temp.pavadzime = k_pavadzime.id WHERE 1=1 ";
		}
		if ($this->MyPOST[pavadzime]) {
			$this->DB->query("DELETE FROM k_balkis_temp");
		...
		if($this->XML_Variants) 
		{
			...
			for ($i=0;$i<$this->lsk;$i++)
			{
				...
				if($this->MyPOST["gtype".$this->grup.($i+1)]=="k_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['k_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
function print_level($level,$query,&$summa_p,$irbrakis_p,$suga,$id)
{
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'k_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'skira' || $lauks=='suga' || $lauks=='iecirknis')
...
//Smiltene Impex ipasa grupesana pec mind_miza ... 
 ...
 //       $this->MyPOST["gtype".$this->grup.$level] = 'mind_miza';
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . " >= " . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . " <= " . $mbounds[1] . " AND k_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . " >= " . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . " <= " . $high_range_brakis . " AND k_balkis_temp.import_type = 'zbm'))";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Saakums////////////////////////////////////////////////////////////////////////////////////
/*Query izvilksana*/  
      $query__="select tilpums_scan as tilpums_neto,tilpums as tilpums_bruto, garums as LBruto,k_balkis_temp.id as bb_id, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, k_pavadzime.cenu_matrica as cm" . $myquery;
...
//          $LNomBruto = (nom($m['LBruto'],$virsmeri,$min_virsmers))/100;
          ...
          $tilpums_bruto = $m['tilpums_bruto'] / 10000;
          $tilpums_neto = $m['tilpums_neto'] / 10000;
...
//          $LNomBruto = (nom($m['LBruto'],$virsmeri,$min_virsmers))/100;
          ...
          $tilpums_bruto = ROUND($m['tilpums_bruto'] / 10000,3);
          $tilpums_neto = ROUND($m['tilpums_neto'] / 10000,3);
