...
$braki = gAtskaite::getBrakiPata();
$LVM_braki = gAtskaite::getLVMBraki();
$VIKA_braki = gAtskaite::getVIKABrakiPata();
...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
			...
			if(trim($this->MyPOST['pavadzime'])!='')
				...
				if	(!GetFromPavadzime($this->MyPOST['pavadzime'],'pavadzime','pata_ab_pavadzime') ) $this->ERRORS .= "Norādīts NEEKSISTĒJOŠS PAVADZĪMES numurs!<br><br>"; 
	...
	{	
		...
		{
			$pav_test_query = "select count(*) as x from pata_ab_pavadzime where pavadzime = '".trim($this->MyPOST['pavadzime'])."'";
		...
		if ($this->MyPOST[datums_no_diena]!='' || $this->MyPOST[pavadzime_id]) { // 
			$query = " from pata_ab_balkis_temp INNER JOIN pata_ab_pavadzime ON pata_ab_balkis_temp.pavadzime = pata_ab_pavadzime.id WHERE 1=1 ";
		} else {
			$query = " from pata_ab_balkis_temp INNER JOIN pata_ab_pavadzime ON pata_ab_balkis_temp.pavadzime = pata_ab_pavadzime.id WHERE 1=1 ";
		}
		...
		if ($this->MyPOST[piegad_grupa])
			$query=$query . " and pata_ab_pavadzime.piegad_grupa like '".$this->MyPOST[piegad_grupa]."%'";
		if ($this->MyPOST[piegadataju_kods])
			$query=$query . " and pata_ab_pavadzime.piegadataju_kods = ".$this->MyPOST[piegadataju_kods];
		if ($this->MyPOST[pavadzime])
			$query=$query . " and pata_ab_pavadzime.pavadzime = '".$this->MyPOST[pavadzime]."'";
		if ($this->MyPOST[pavadzime_id])
			$query=$query . " and pata_ab_pavadzime.id = '".$this->MyPOST[pavadzime_id]."'";
		if ($this->MyPOST[suga]!='') 
		...
		$query2 = str_replace("pata_ab_balkis_temp","pata_ab_balkis",$query);

		if ($this->MyPOST[datums_no_diena]!='') {
			$this->DB->query("DELETE FROM pata_ab_balkis_temp");
			$this->DB->query("INSERT INTO pata_ab_balkis_temp (SELECT pata_ab_balkis.* " . $query2 . ")");
			$query = " from pata_ab_balkis_temp INNER JOIN pata_ab_pavadzime ON pata_ab_balkis_temp.pavadzime = pata_ab_pavadzime.id WHERE 1=1 ";
		}
		if ($this->MyPOST[pavadzime]) {
			$this->DB->query("DELETE FROM pata_ab_balkis_temp");
...
//						echo "<br> IN mind_pirms_red "."gtype".($i+1)."invalue [".(($this->MyPOST["gvalues".($i+1)]) || ($this->MyPOST["gvalues".($i+1)."_1"]) || ($this->MyPOST["gvalues".($i+1)."_2]"]))."]";	
				...
				if($this->MyPOST["gtype".$this->grup.($i+1)]=="pata_ab_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['pata_ab_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
//    echo "Lauks = ".$lauks.'<br />';
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'pata_ab_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'skira' || $lauks=='suga' || $lauks=='iecirknis')
...
//			$this->MyPOST["gtype".$this->grup.$level] = 'mind_miza';
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . " >= " . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . " <= " . $mbounds[1] . " AND pata_ab_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . " >= " . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . " <= " . $high_range_brakis . " AND pata_ab_balkis_temp.import_type = 'zbm'))";
...
//      echo "Brakis = ".$irbrakis.'<br />';
      ...
      $query__="select mind_pirms_red as tievgalis_pirms_redukcijas,mind_miza as mind_miza, maxd_miza as resgalis_pirms_redukcijas, garums as LBruto,pata_ab_balkis_temp.id as bb_id,mind_pirms_red as DPirmsReduc, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, pata_ab_pavadzime.cenu_matrica as cm" . $myquery;
      $r = $this->DB->query($query__);
...
//        $DPecReduc = $m['DPecReduc']/1000;
        $DPecReduc = ($m['mind_miza'] - $Diametra_starpiba)/1000;
//echo $Diametra_starpiba.'<br />';
        $DReduc = $DBruto-$DPecReduc;

        if ($this->MyPOST['noapalot_diametru'] == 'on') {
          $DBrutoNoap = ((floor($m['mind_miza']/10)+0.5)/100);
        } else {
          $DBrutoNoap = $m['mind_miza']/1000;
        }
...
/*          
          echo "Balka ID =".$idents.'<br />';
          echo "Tilpums Neto = ".$tilpums_neto.'<br />';
          echo "Tilpums Bruto Bez Virsm. = ".$tilpums_bruto_virsmeram.'<br />';
          echo "Tilpums Bruto = ".$tilpums_bruto.'<br />';
          echo "Tilpums Virsmēram = ".$tilpums_virsmers.'<br />';
          echo "Tilpums Redukcijai = ".$tilpums_kopejai_redukcijai.'<br />';
          echo "-----------------------------------------------------------------".'<br />';
          echo "Lielums DBrutoNoap =".$DBrutoNoap.'<br />';
          echo "Lielums LBruto =".$LBruto.'<br />'.'<br />';
          echo "Lielums DPecReduc =".$DPecReduc.'<br />';
          echo "Lielums LnomNeto =".$LnomNeto.'<br />'.'br /';
          echo "Lielums LNomBruto =".$LNomBruto.'<br />'.'<br />'.'<br />';
*/
...
/*          
          ...
          echo "Tilpums Bruto = ".$tilpums_bruto.'<br />';
          echo "Tilpums Virsmēram = ".$tilpums_virsmers.'<br />';
          echo "Tilpums Redukcijai = ".$tilpums_kopejai_redukcijai.'<br />'.'<br />';
          echo "Lielums DBrutoNoap =".$DBrutoNoap.'<br />';
          echo "Lielums LBruto =".$LBruto.'<br />'.'<br />';
          echo "Lielums DPecReduc =".$DPecReduc.'<br />';
          echo "Lielums LnomNeto =".$LnomNeto.'<br />'.'<br />';
