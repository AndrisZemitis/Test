...
$braki = gAtskaite::getVIKABrakiIncukalns();
$LVM_braki = gAtskaite::getLVMBraki();
$IT_braki = gAtskaite::getITBraki();
$VIKA_braki = gAtskaite::getVIKABrakiIncukalns();
$VIKA_LVM_braki = gAtskaite::getVIKABrakiIncukalns();
...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
	...
	{	
		...
		{
			$pav_test_query = "select count(*) as x from i_pavadzime where pavadzime = '".trim($this->MyPOST['pavadzime'])."'";
		...
		if ($this->MyPOST[datums_no_diena]!='' || $this->MyPOST[pavadzime_id]) { // 
			$query = " from i_balkis_temp INNER JOIN i_pavadzime ON i_balkis_temp.pavadzime = i_pavadzime.id WHERE 1=1 ";
		} else {
			$query = " from i_balkis INNER JOIN i_pavadzime ON i_balkis.pavadzime = i_pavadzime.id WHERE 1=1 ";
		}
		...
		if ($this->MyPOST[piegad_grupa])
			$query=$query . " and i_pavadzime.piegad_grupa like '".$this->MyPOST[piegad_grupa]."%'";
		if ($this->MyPOST[piegadataju_kods])
			$query=$query . " and i_pavadzime.piegadataju_kods = ".$this->MyPOST[piegadataju_kods];
		if ($this->MyPOST[pavadzime])
			$query=$query . " and i_pavadzime.pavadzime = '".$this->MyPOST[pavadzime]."'";
		if ($this->MyPOST[pavadzime_id])
			$query=$query . " and i_pavadzime.id = '".$this->MyPOST[pavadzime_id]."'";
		if ($this->MyPOST[suga]!='') 
		...
		$query2 = str_replace("i_balkis_temp","i_balkis",$query);

		if ($this->MyPOST[datums_no_diena]!='') {
			$this->DB->query("DELETE FROM i_balkis_temp");
			$this->DB->query("INSERT INTO i_balkis_temp (SELECT i_balkis.* " . $query2 . ")");
			$query = " from i_balkis_temp INNER JOIN i_pavadzime ON i_balkis_temp.pavadzime = i_pavadzime.id WHERE 1=1 ";
		}
		if ($this->MyPOST[pavadzime]) {
			$this->DB->query("DELETE FROM i_balkis_temp");
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)]=str_replace("b0-219","b60-99,b100-119,b120-139,b140-159,b160-179,b180-199,b200-219",$this->MyPOST["gvalues".$this->grup.($i+1)]);
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_2"],2);
					...
					if($this->MyPOST["gtype".$this->grup.($i+1)]=="i_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['i_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
function print_level($level,$query,&$summa_p,$irbrakis_p,$suga,$id)
{
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'i_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'suga' || $lauks=='iecirknis' || ($lauks == 'skira' && $a[0] == '' && sizeof($abr) == 0))
...
//pievienots 17.07.2007
			if ($this->MyPOST["gtype".$this->grup.$level] == 'mind_pirms_red') {
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $mbounds[1] . " AND i_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $high_range_brakis . " AND i_balkis_temp.import_type = 'zbm'))";
			} else {
...
/*Query izvilksana*/  
      $query__="select i_balkis_temp.id as Ident, mind_pirms_red as DBruto, garums as LBruto, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, i_pavadzime.cenu_matrica as cm " . $myquery;
...
//          echo $LnomNeto.'<br />';
          ...
          $tilpums_diametrs_redukcija = $tilpums_bruto_virsmeram - $tilpums_neto;
...
//-------------------------------------------TEST-Line->
      ...
      $query__="select mind_pirms_red as DBruto, garums as LBruto, mind_pec_red as DPecReduc,mind_miza as vidus_diam, gar_pec_red as LPecReduc, brakis, i_pavadzime.cenu_matrica as cm, i_balkis_temp.id as r_id" . $myquery;
...
//					echo "Bruto noapaļots = ".$DBrutoNoap.'<br>';
        ...
        $DPecReduc = $m['DPecReduc']/1000;
        $DReduc = $DBruto-$DPecReduc;
        $DPecReduc = ($m['vidus_diam']/1000) - $DReduc;
...
//        echo "VReducD => ".$VReducD." = ".$VBruto." - ".$VBezReducD.'<br/>','<br/>','<br/>';
		...
		if ($this->MyPOST['noapalot_diametru'] == 'on') {
			$DBrutoNoap = ((floor($m['vidus_diam']/10)+0.5)/100);
		} else {
			$DBrutoNoap = $m['vidus_diam']/1000;
		}
...
/*3.metode<------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*Query izvilksana*/  
      $query__="select mind_pirms_red as tievgalis_pirms_redukcijas,maxd_miza as resgalis_pirms_redukcijas, garums as LBruto,balkis_temp.id as bb_id,mind_pirms_red as DPirmsReduc, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, i_pavadzime.cenu_matrica as cm" . $myquery;
      $r = $this->DB->query($query__);
...
/*4.metode<------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*Query izvilksana*/  
      $query__="select mind_pirms_red as DBruto, garums as LBruto, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, i_pavadzime.cenu_matrica as cm" . $myquery;
//			echo $query__.'<br />';
			$r = $this->DB->query($query__);
/*NULL vertibu ievietosana*/			
      $SVBruto = 0;
