...
function getMaskGroup($var_MaskString,$findString,$groupType){
  ...
  foreach($tmp_arrCompare as $newItem){
    if($groupType != 'suga' && $groupType != 'skira'){
      ...
      if($var_MaskString != '' && (($tmp_arrCompare[0] == $findString) || ($tmp_arrCompare[1] == $findString))){
...
function constrByMask($pvdzID){
  ...
  $tmp_arrCollName['atgr_brakis'] = "Atgr. brāķis";
  ...
  //-#001-FUNC-START--Nepieciešamo datu kolekcionēšana no datubāzes------------------------------------------------------------------------------------
      ...
      $var_BrakisKods = $tmp_getDataQuery_arr['brakis'];
  ...
  //-#007-FUNC-START--Statisko vērtību ievietošana---------------------------------------------------------------------------------------------------    
      ...
      if($this->arrPrintOut[$insertRow]['brakis_kods'] != ''){
        $isAtgBrakis = chkIfRejectSndBack($this->firmCode,$var_BrakisKods);
  ...
  //-#008-FUNC-START--Dinamisko vērtību piešķiršana---------------------------------------------------------------------------------------------------    
      ...
      if($isAtgBrakis == 0){
        ...
        $this->arrPrintOut[$insertRow]['atgr_brakis'] += 0;
      }else{
        $this->arrPrintOut[$insertRow]['brakis'] += 0;
        $this->arrPrintOut[$insertRow]['atgr_brakis'] += $tilpums_brakis;
      }
  ...
  //------------------------------------------------------------------------------------------------------
      ...
      if($this->arrPrintOut[$insertRow]['atgr_brakis'] != ''){$this->arrPrintOut[$insertRow]['atgr_brakis'] = number_format($this->arrPrintOut[$insertRow]['atgr_brakis'], 3, '.', '');}else{$this->arrPrintOut[$insertRow]['atgr_brakis'] = '';}
  ...
  //------------------------------------------------------------------------------------------------------
      ...
      if($isAtgBrakis == 0){
        ...
        $tilpums_atg_brakis_KOPA = $tilpums_atg_brakis_KOPA + $tilpums_brakis;
      }
...
//Kopsavilkuma atskaites papildnosacījums
  }else{
...
*/    
      ...
      if($isAtgBrakis == 0){
        $tilpums_brakis_atg = 0;
      }else{
        $tilpums_brakis_atg = $tilpums_brakis;
        $tilpums_brakis = 0;
      }

      $tmp_skaits = 1;
      $querySub_text = "INSERT INTO `global_kops_atsk` (`group_suga`,`group_skira`,`group_diametrs`,`group_garums`,`group_brakis`,`group_tilp_neto`,`group_tilp_bruto`,`group_tilp_brakis`,`group_tilp_brakis_atg`,`group_tilp_neto_un_brakis`,`group_tilp_redukcija`,`group_tilp_virsmers`,`group_skaits`) VALUES ('$prnt_Suga','$prnt_Skira','$prnt_Diametrs','$prnt_Garums','$prnt_Brakis',$tilpums_neto,$tilpums_bruto,$tilpums_brakis,$tilpums_brakis_atg,$tilpums_neto + $tilpums_brakis,$tilpums_redukcija,$tilpums_virsmers,$tmp_skaits)";
...
//    echo $mysqlGlobalSelect_text.'<br />';
    ...
    while($mysqlGlobalSelect_arr = mysql_fetch_assoc($mysqlGlobalSelect)){
      ...
      $this->arrPrintOut[$insertRow]['atgr_brakis'] = $mysqlGlobalSelect_arr['brakis_atg_tilp'];
...
//-#010-FUNC-START--Kopsummas rindas pievienošana---------------------------------------------------------------------------------------------------    
    ...
    $this->arrPrintOut[$tmp_rowCount]['atgr_brakis'] = number_format($tilpums_atg_brakis_KOPA, 3, '.', '')."*";
    $this->arrPrintOut[$tmp_rowCount]['neto'] = number_format($tilpums_neto_KOPA, 3, '.', '')."*";
    $this->arrPrintOut[$tmp_rowCount]['brakis_un_neto'] = number_format($tilpums_atg_brakis_KOPA + $tilpums_brakis_KOPA + $tilpums_neto_KOPA, 3, '.', '')."*";
    ...
    $this->regSuperSum['brakis'] = $tilpums_brakis_KOPA + $tilpums_atg_brakis_KOPA;
...
//----------------------------------------------------------------------------------------
      ...
      $output = $output . "		<Name>".$reciever_name."</Name>\n";
    ...
    foreach($this->arrPrintOut as $tmp_item_1){
     ...
     if($tmp_item_1['nosaukums'] != 1){
        ...
        if($refused){
          ...
          if($tmp_item_1['brakis_kods'] == 'AK'){
            $volume = round($tmp_item_1['atgr_brakis'],3);
          }
        } else {
...
//------------------------------------------------------------------------------------------
        ...
        if($atskaite_veids == 33 || $atskaite_veids == 29 || $atskaite_veids == 28 || $atskaite_veids == 7 || $atskaite_veids == 32 || $atskaite_veids == 36){
          if($tmp_skira == 3){$datarow_sizegroup = $datarow_sizegroup."(3)";}
        }

        if($atskaite_veids == 16 || $atskaite_veids == 34 || $atskaite_veids == 30){
          if($tmp_skira == 1){$datarow_sizegroup = "A".$datarow_sizegroup; $datarow_assortment = "A";}
        }

        if($atskaite_veids == 26 || $atskaite_veids == 35 || $atskaite_veids == 31){
...
//          $output = $output .  "	<Length>".$datarow_length."</Length>\n";
      ...
      $sumBrakiskopa = $sumBrakiskopa + round($tmp_item_1['brakis'],3) + round($tmp_item_1['atgr_brakis'],3);
...
function mod_ResignLVMRejectCode($atskaite_num,$tmpr_Garums,$tmpr_Diametrs,$tmpr_Suga,$tmp_Skira){
  $tmp_result = false;
  ...
  switch($atskaite_num){
    ...
    break;
    case 11:
      if (($tmpr_Diametrs >= 90 && $tmpr_Diametrs <= 299) && ($tmpr_Garums >= 300 && $tmpr_Garums < 615)){
        $tmp_result = true;
      }
    break;
    case 25:
    ...
    break;
    case 21:
      if((($tmpr_Suga == 1) && ($tmpr_Diametrs >= 100) && ($tmpr_Garums >= 306 && $tmpr_Garums <= 631)) || (($tmpr_Suga == 2) && ($tmpr_Diametrs >= 100) && ($tmpr_Garums >= 306 && $tmpr_Garums <= 635))){
        $tmp_result = true;
      }
    break;
    case 28:
      if ((($tmpr_Diametrs >= 60 && $tmpr_Diametrs <= 99) && ($tmpr_Garums >= 340 && $tmpr_Garums <= 365)) || (($tmpr_Diametrs >= 100 && $tmpr_Diametrs <= 119) && ($tmpr_Garums >= 340 && $tmpr_Garums <= 515)) || (($tmpr_Diametrs >= 120 && $tmpr_Diametrs <= 219) && ($tmpr_Garums >= 370 && $tmpr_Garums <= 515))){
        $tmp_result = true;
      }
    break;
    case 27:
      if (($tmpr_Diametrs >= 300 && $tmpr_Diametrs <= 500) && ($tmpr_Garums >= 307 && $tmpr_Garums < 632) && $tmpr_Suga == 1){
        $tmp_result = true;
      }
    break;
    case 35:
    ...
    break;
    case 31:
      if (($tmpr_Diametrs >= 170 && $tmpr_Diametrs <= 700) && ($tmpr_Garums >= 275 && $tmpr_Garums <= 335)){
        $tmp_result = true;
      }
    break;
    case 36:
      if (($tmpr_Diametrs >= 160 && $tmpr_Diametrs <= 600) && ($tmpr_Garums >= 300 && $tmpr_Garums <= 335)){
        $tmp_result = true;
      }
    break;
    case 30:
      if ((($tmpr_Diametrs >= 240 && $tmpr_Diametrs <= 800) && ($tmpr_Garums >= 480 && $tmpr_Garums <= 635) && $tmp_Skira == 4) && (($tmpr_Diametrs >= 240 && $tmpr_Diametrs <= 800) && ($tmpr_Garums >= 300 && $tmpr_Garums <= 635) && $tmp_Skira == 1)){
        $tmp_result = true;
      }
    break;
  }
...
function chkIfRejectSndBack($atskaite_num, $defCode){
...
*/
      ...
      case 23:
        $braki['000']['LAT'] = 'Metāls';
        $braki['001']['LAT'] = 'Metāls';
        $braki['002']['LAT'] = 'Papīrmalka';//Papīrmalka
        $braki['003']['LAT'] = 'Dimensija';//Par resnu tievgalis
        $braki['004']['LAT'] = 'Trupe';//Trupe, lielainums
        $braki['005']['LAT'] = 'Saussānis';//Saussānis
        $braki['006']['LAT'] = 'Zari';//Zari
        $braki['007']['LAT'] = 'Līkumainība';//Līkumainība
        $braki['008']['LAT'] = 'Citi';//Citi
        $braki['009']['LAT'] = 'Dimensija';//Par īsu
        $braki['010']['LAT'] = 'Dimensija';//Par tievu
        $braki['012']['LAT'] = 'Līkumainība';//Līkumainība
        $braki['016']['LAT'] = 'Dimensija';//Diam. neatb.
        $braki['017']['LAT'] = 'Dimensija';//Gar. neatb.
        $braki['255']['LAT'] = '';
			break;
      case 26:
        $braki['000']['LAT'] = 'Metāls';
...
*/        
    ...
    switch($atskaite_num){
      case 12:
        $braki['000']['LAT'] = 'C'; //Tukšais brāķis
        $braki['001']['LAT'] = 'C';
        $braki['002']['LAT'] = 'M';
        $braki['003']['LAT'] = 'C';
        $braki['004']['LAT'] = 'L';
