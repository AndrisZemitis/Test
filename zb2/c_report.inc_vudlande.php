...
$VIKA_braki = gAtskaite::getVIKABrakiVudlande();
$VIKA_LVM_braki = gAtskaite::getVIKALVMBrakiVudlande();
...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
			...
			if(trim($this->MyPOST['pavadzime'])!='')
				{
				if	(!GetFromPavadzime($this->MyPOST['pavadzime'],'pavadzime','vudlande_pavadzime') ) $this->ERRORS .= "Norādīts NEEKSISTĒJOŠS PAVADZĪMES numurs!<br><br>"; 
	...
	{	
		...
		{
			$pav_test_query = "select count(*) as x from vudlande_pavadzime where pavadzime = '".trim($this->MyPOST['pavadzime'])."'";
		...
		if ($this->MyPOST[datums_no_diena]!='' || $this->MyPOST[pavadzime_id]) { // 
			$query = " from vudlande_balkis_temp INNER JOIN vudlande_pavadzime ON vudlande_balkis_temp.pavadzime = vudlande_pavadzime.id WHERE 1=1 ";
		} else {
			$query = " from vudlande_balkis INNER JOIN vudlande_pavadzime ON vudlande_balkis.pavadzime = vudlande_pavadzime.id WHERE 1=1 ";
		}
		...
		if ($this->MyPOST[piegad_grupa])
			$query=$query . " and vudlande_pavadzime.piegad_grupa like '".$this->MyPOST[piegad_grupa]."%'";
		if ($this->MyPOST[piegadataju_kods])
			$query=$query . " and vudlande_pavadzime.piegadataju_kods = ".$this->MyPOST[piegadataju_kods];
		if ($this->MyPOST[pavadzime])
			$query=$query . " and vudlande_pavadzime.pavadzime = '".$this->MyPOST[pavadzime]."'";
		if ($this->MyPOST[pavadzime_id])
			$query=$query . " and vudlande_pavadzime.id = '".$this->MyPOST[pavadzime_id]."'";
		if ($this->MyPOST[suga]!='') 
		...
		$query2 = str_replace("vudlande_balkis_temp","vudlande_balkis",$query);

		if ($this->MyPOST[datums_no_diena]!='') {
			$this->DB->query("DELETE FROM vudlande_balkis_temp");
			$this->DB->query("INSERT INTO vudlande_balkis_temp (SELECT vudlande_balkis.* " . $query2 . ")");
			$query = " from vudlande_balkis_temp INNER JOIN vudlande_pavadzime ON vudlande_balkis_temp.pavadzime = vudlande_pavadzime.id WHERE 1=1 ";
		}
		if ($this->MyPOST[pavadzime]) {
			$this->DB->query("DELETE FROM vudlande_balkis_temp");
...
//			echo "Kverijs 2 = ","INSERT INTO vudlande_balkis_temp (SELECT vudlande_balkis.* " . $query2 . ")",'<br />';
			$query = " from vudlande_balkis_temp INNER JOIN vudlande_pavadzime ON vudlande_balkis_temp.pavadzime = vudlande_pavadzime.id WHERE 1=1 ";
		}
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)]);
						$this->MyPOST["gvalues".$this->grup.($i+1)]=str_replace("b0-259","b0-59,b60-99,b100-119,b120-139,b140-159,b160-179,b180-199,b200-219,b220-239,b240-259",$this->MyPOST["gvalues".$this->grup.($i+1)]);
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_1"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_1"],1);
						$this->MyPOST["gvalues".$this->grup.($i+1)."_1"]=str_replace("b0-259","b0-59,b60-99,b100-119,b120-139,b140-159,b160-179,b180-199,b200-219,b220-239,b240-259",$this->MyPOST["gvalues".$this->grup.($i+1)."_1"]);
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_2"],2);
						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=str_replace("b0-259","b0-59,b60-99,b100-119,b120-139,b140-159,b160-179,b180-199,b200-219,b220-239,b240-259",$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]);
				...
				if($this->MyPOST["gtype".$this->grup.($i+1)]=="vudlande_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['vudlande_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
function print_level($level,$query,&$summa_p,$irbrakis_p,$suga,$id)
{
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'vudlande_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'skira' || $lauks=='suga' || $lauks=='iecirknis')
...
//pievienots 17.07.2007
			if ($this->MyPOST["gtype".$this->grup.$level] == 'mind_pirms_red') {
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $mbounds[1] . " AND vudlande_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $high_range_brakis . " AND vudlande_balkis_temp.import_type = 'zbm'))";
			} else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Saakums////////////////////////////////////////////////////////////////////////////////////
/*Query izvilksana*/  
      $query__="select mind_pirms_red as DBruto,vudlande_balkis_temp.id as r_id, garums as LBruto, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, vudlande_pavadzime.cenu_matrica as cm " . $myquery;
