...
$braki = gAtskaite::getBrakiNelss();
$LVM_braki = gAtskaite::getLVMBraki();
$IT_braki = gAtskaite::getITBrakiNelss();
$VIKA_braki = gAtskaite::getVIKABrakiPiebalgas();
$VIKA_LVM_braki = gAtskaite::getVIKALVMBrakiPiebalgas();
...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
			...
			if(trim($this->MyPOST['pavadzime'])!='')
				{
				if	(!GetFromPavadzime($this->MyPOST['pavadzime'],'pavadzime','piebalgas_pavadzime') ) $this->ERRORS .= "Norādīts NEEKSISTĒJOŠS PAVADZĪMES numurs!<br><br>"; 
	...
	{	
		...
		{
			$pav_test_query = "select count(*) as x from piebalgas_pavadzime where pavadzime = '".trim($this->MyPOST['pavadzime'])."'";
		...
		if ($this->MyPOST[datums_no_diena]!='' || $this->MyPOST[pavadzime_id]) { // 
			$query = " from piebalgas_balkis_temp INNER JOIN piebalgas_pavadzime ON piebalgas_balkis_temp.pavadzime = piebalgas_pavadzime.id WHERE 1=1 ";
		} else {
			$query = " from piebalgas_balkis INNER JOIN piebalgas_pavadzime ON piebalgas_balkis.pavadzime = piebalgas_pavadzime.id WHERE 1=1 ";
		}
		...
		if ($this->MyPOST[piegad_grupa])
			$query=$query . " and piebalgas_pavadzime.piegad_grupa like '".$this->MyPOST[piegad_grupa]."%'";
		if ($this->MyPOST[piegadataju_kods])
			$query=$query . " and piebalgas_pavadzime.piegadataju_kods = ".$this->MyPOST[piegadataju_kods];
		if ($this->MyPOST[pavadzime])
			$query=$query . " and piebalgas_pavadzime.pavadzime = '".$this->MyPOST[pavadzime]."'";
		if ($this->MyPOST[pavadzime_id])
			$query=$query . " and piebalgas_pavadzime.id = '".$this->MyPOST[pavadzime_id]."'";
		if ($this->MyPOST[suga]!='') 
		...
		$query2 = str_replace("piebalgas_balkis_temp","piebalgas_balkis",$query);

		if ($this->MyPOST[datums_no_diena]!='') {
			$this->DB->query("DELETE FROM piebalgas_balkis_temp");
			$this->DB->query("INSERT INTO piebalgas_balkis_temp (SELECT piebalgas_balkis.id,piebalgas_balkis.pavadzime,piebalgas_balkis.nelieto,piebalgas_balkis.datums_laiks,piebalgas_balkis.mind_pirms_red,piebalgas_balkis.garums,piebalgas_balkis.suga,piebalgas_balkis.skira,piebalgas_balkis.miza,piebalgas_balkis.mind_pec_red,piebalgas_balkis.gar_pec_red,piebalgas_balkis.mind_miza,IF(piebalgas_balkis.gar_pec_red < 370 AND piebalgas_balkis.skira < 9,'010',piebalgas_balkis.brakis),piebalgas_balkis.maxd_miza,piebalgas_balkis.kabata,piebalgas_balkis.tilpums,piebalgas_balkis.tilpums_scan,piebalgas_balkis.import_type,piebalgas_balkis.import_id " . $query2 . ")");
			$query = " from piebalgas_balkis_temp INNER JOIN piebalgas_pavadzime ON piebalgas_balkis_temp.pavadzime = piebalgas_pavadzime.id WHERE 1=1 ";
		}
		if ($this->MyPOST[pavadzime]) {
			$this->DB->query("DELETE FROM piebalgas_balkis_temp");
...
//			echo "Kverijs 2 = ","INSERT INTO piebalgas_balkis_temp (SELECT piebalgas_balkis.* " . $query2 . ")",'<br />';
			$query = " from piebalgas_balkis_temp INNER JOIN piebalgas_pavadzime ON piebalgas_balkis_temp.pavadzime = piebalgas_pavadzime.id WHERE 1=1 ";
		}
			...
			for ($i=0;$i<$this->lsk;$i++)
			{
				...
				if($this->XML_Variants)
				{
					...
					if($this->MyPOST["gtype".$this->grup.($i+1)]=='mind_pirms_red' ) {
						...
						$this->MyPOST["gvalues".$this->grup.($i+1)]=str_replace("b0-99","b0-59,b60-99",$this->MyPOST["gvalues".$this->grup.($i+1)]);
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_1"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_1"],1);
						$this->MyPOST["gvalues".$this->grup.($i+1)."_1"]=str_replace("b0-99","b0-59,b60-99",$this->MyPOST["gvalues".$this->grup.($i+1)."_1"]);
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_2"],2);
						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=str_replace("b0-99","b0-59,b60-99",$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]);
				...
				if($this->MyPOST["gtype".$this->grup.($i+1)]=="piebalgas_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['piebalgas_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
function print_level($level,$query,&$summa_p,$irbrakis_p,$suga,$id)
{
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'piebalgas_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'skira' || $lauks=='suga' || $lauks=='iecirknis')
...
//pievienots 17.07.2007
			if ($this->MyPOST["gtype".$this->grup.$level] == 'mind_pirms_red') {
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $mbounds[1] . " AND piebalgas_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $high_range_brakis . " AND piebalgas_balkis_temp.import_type = 'zbm'))";
			} else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Saakums////////////////////////////////////////////////////////////////////////////////////
/*Query izvilksana*/  
      $query__="select mind_pirms_red as DBruto,piebalgas_balkis_temp.id as r_id, garums as LBruto, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, piebalgas_pavadzime.cenu_matrica as cm " . $myquery;
