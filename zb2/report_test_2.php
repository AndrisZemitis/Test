...
?>
  ...
  <script type="text/javascript" src="../jQuery/sablonu_grupesana.js"></script>

<center>
<form name=forma method=POST id="forma" action="report_view_test_2.php?veids=<?=$veids?>&h=<?=$h?>" target=_blank>
...
<table bgcolor=#BFDBBF>
	...
	<tr>
        ...
        <option value="13" <? if($tpl['parbaudija_head']==13){echo " selected";}else{get_param_selected('parbaudija','13');}?>>Gunta Ziemele</option>
		  </select>
		</td>
	</tr>
	<tr>
		...
		<td align=left>
		  <select name=parbaudija_un_atbildigais_head>
		    <option value="6" <? if($tpl['parbaudija_un_atbildigais_head']==6){echo " selected";}else{get_param_selected('parbaudija_atbildigais','6');}?>></option>
              ...
              <option value="15" <?  if($tpl['parbaudija_un_atbildigais_head']==15){echo " selected";}else{get_param_selected('parbaudija_atbildigais','15');}?>>Lotārs Auziņš</option>
		  </select>
		</td>
	</tr>
	<tr>
...
Meklēt: <input name="filter" id="filter" value="" maxlength="30" size="30" type="text"><br /><br />
<table class="sablonu_filtrs" bgcolor="#FDFFCF">
	<tr>
		<td>ID&nbsp;</td>
		<td>Nosaukums&nbsp;</td>
		<td>Informācija&nbsp;</td>
  ...
  ?>
	<tr>
		...
		<td><a href="http://www.vmf.lv/zb2/report_test_2.php?h=<? echo $h."&template=".$m_sabloni['id'];?>">Izvēlēties</a></td>
		<td>
...
<table>
<tr>
	<td align=center colspan=2><input type="button" onclick="var f=document.getElementById('forma');f.action='report_view_test_2.php?veids=<?=$veids?>&h=<?=$h?>';f.submit();"" name=poga value="    Veidot    "><input type="button" onclick="var f=document.getElementById('forma');f.action='report_view_test_2.php?xml=1&veids=<?=$veids?>&h=<?=$h?>';f.submit();" name="poga_xml" value="    Veidot XML   "></td>
</tr>
</table>
<br>
