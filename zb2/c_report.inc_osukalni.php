...
//			if(trim($this->MyPOST['pieg_lig_num'])=='') $this->ERRORS.= "Nav norādīts PIEGĀDES LĪGUMA numurs!<br><br>";
			...
			if(trim($this->MyPOST['pavadzime'])!='')
				{
				if	(!GetFromPavadzime($this->MyPOST['pavadzime'],'pavadzime','osukalni_pavadzime') ) $this->ERRORS .= "Norādīts NEEKSISTĒJOŠS PAVADZĪMES numurs!<br><br>"; 
	...
	{	
		...
		{
			$pav_test_query = "select count(*) as x from osukalni_pavadzime where pavadzime = '".trim($this->MyPOST['pavadzime'])."'";
		...
		if ($this->MyPOST[datums_no_diena]!='' || $this->MyPOST[pavadzime_id]) { // 
			$query = " from osukalni_balkis_temp INNER JOIN osukalni_pavadzime ON osukalni_balkis_temp.pavadzime = osukalni_pavadzime.id WHERE 1=1 ";
		} else {
			$query = " from osukalni_balkis INNER JOIN osukalni_pavadzime ON osukalni_balkis.pavadzime = osukalni_pavadzime.id WHERE 1=1 ";
		}
		...
		if ($this->MyPOST[piegad_grupa])
			$query=$query . " and osukalni_pavadzime.piegad_grupa like '".$this->MyPOST[piegad_grupa]."%'";
		if ($this->MyPOST[piegadataju_kods])
			$query=$query . " and osukalni_pavadzime.piegadataju_kods = ".$this->MyPOST[piegadataju_kods];
		if ($this->MyPOST[pavadzime])
			$query=$query . " and osukalni_pavadzime.pavadzime = '".$this->MyPOST[pavadzime]."'";
		if ($this->MyPOST[pavadzime_id])
			$query=$query . " and osukalni_pavadzime.id = '".$this->MyPOST[pavadzime_id]."'";
		if ($this->MyPOST[suga]!='') 
		...
		$query2 = str_replace("osukalni_balkis_temp","osukalni_balkis",$query);

		if ($this->MyPOST[datums_no_diena]!='') {
			$this->DB->query("DELETE FROM osukalni_balkis_temp");
			$this->DB->query("INSERT INTO osukalni_balkis_temp (SELECT osukalni_balkis.* " . $query2 . ")");
			$query = " from osukalni_balkis_temp INNER JOIN osukalni_pavadzime ON osukalni_balkis_temp.pavadzime = osukalni_pavadzime.id WHERE 1=1 ";
		}
		if ($this->MyPOST[pavadzime]) {
			$this->DB->query("DELETE FROM osukalni_balkis_temp");
...
//			echo "Kverijs 2 = ","INSERT INTO osukalni_balkis_temp (SELECT osukalni_balkis.* " . $query2 . ")",'<br />';
			$query = " from osukalni_balkis_temp INNER JOIN osukalni_pavadzime ON osukalni_balkis_temp.pavadzime = osukalni_pavadzime.id WHERE 1=1 ";
		}
			...
			for ($i=0;$i<$this->lsk;$i++)
			{
				...
				if($this->XML_Variants)
				{
					...
					if($this->MyPOST["gtype".$this->grup.($i+1)]=='mind_pirms_red' ) {
						...
						$this->MyPOST["gvalues".$this->grup.($i+1)]=str_replace("b0-159","b0-59,b60-99,b100-119,b120-139,b140-159",$this->MyPOST["gvalues".$this->grup.($i+1)]);
...
//						$this->MyPOST["gvalues".$this->grup.($i+1)."_2"]=LVM_distribution($this->MyPOST["gvalues".$this->grup.($i+1)."_2"],2);
				...
				if($this->MyPOST["gtype".$this->grup.($i+1)]=="osukalni_pavadzime.pavadzime") $this->arr[-1][$i]=$this->mtipi['osukalni_pavadzime.pavadzime']; //kaut k?ds exception
				}
			}
			//print_r($this->MyPOST);
...
function print_level($level,$query,&$summa_p,$irbrakis_p,$suga,$id)
{
	...
	if ($lauks == 'kad_piegad' || $lauks == 'auto' || $lauks == 'brakis' || $lauks == 'cenu_matrica' || $lauks == 'cirsmas_kods' || $lauks == 'kad_piegad' || $lauks == 'kravas_id' || $lauks == 'osukalni_pavadzime.pavadzime' || $lauks == 'piegad_kods' || $lauks == 'soferis' || $lauks == 'skira' || $lauks=='suga' || $lauks=='iecirknis')
...
//pievienots 17.07.2007
			if ($this->MyPOST["gtype".$this->grup.$level] == 'mind_pirms_red') {
				$myquery = $query . " and  ((". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $mbounds[0] ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $mbounds[1] . " AND osukalni_balkis_temp.import_type <> 'zbm') or (". $this->MyPOST["gtype".$this->grup.$level] . ">=" . $low_range_brakis ." and " . $this->MyPOST["gtype".$this->grup.$level] . "<=" . $high_range_brakis . " AND osukalni_balkis_temp.import_type = 'zbm'))";
			} else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Saakums////////////////////////////////////////////////////////////////////////////////////
/*Query izvilksana*/  
      $query__="select mind_pirms_red as DBruto,osukalni_balkis_temp.id as r_id, garums as LBruto, mind_pec_red as DPecReduc, gar_pec_red as LPecReduc, brakis, osukalni_pavadzime.cenu_matrica as cm " . $myquery;
...
//        if (strtoupper($this->MyPOST['pieg_lig_num'])=='VIKA WOOD') {
		if ($this->MyPOST['is_vika'] == 'on') {
          ...
          $tilpums_brakim_no_noap_garuma = f2($DBrutoNoap,$LPecReduc,$rauk_koef,$koeficients,$gostu_tabula);
          ...
          $tilpums_garuma_redukcija = $tilpums_bruto_virsmeram - $tilpums_neto;
        ...
        $SVBruto += $tilpums_bruto;
             ...
             if($this->MyPOST['braka_virsmers'] == 'on'){                           
                  ...
                  $SVBrakis += $tilpums_bruto_virsmeram;
             }else{
...
//                  $SVVirsm += $tilpums_bruto - $tilpums_brakim_no_noap_garuma;
                  $SVBrakis += $tilpums_bruto;
             }
					}else{
              $SVBrakis = $tilpums_bruto;
